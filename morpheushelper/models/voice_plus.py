...
class VoiceMutedLog(db.Base):
    ...
    def create(member: int, member_name: str, timestamp: Optional[datetime] = None) -> "VoiceMutedLog":
        row = VoiceMutedLog(member=member, member_name=member_name, timestamp=timestamp or datetime.utcnow())
        db.add(row)
        return row

    @staticmethod
    def update(member: int, member_name: str, muted_at: datetime):
        for mute in db.query(VoiceMutedLog, member=member):
            if mute.timestamp >= muted_at=member): - timedelta(minutes=1):
                break
        else:
            VoiceMutedLog.create(member, member_name, muted_at)