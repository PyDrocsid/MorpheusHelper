...
async def configure_role(
    ctx: Context, role_name: str, role: Optional[Role], check_assignable: bool = False
):
    ...
    if role is None:
        ...
        await send_to_changelog(
            ctx.guild,
            getattr(translations, "f_log_role_set_" + role_name)(role.name, role.id),
        )


async def get_mute_role(guild: Guild) -> Role:
    mute_role: Optional[Role] = guild.get_role(
        await run_in_thread(Settings.get, int, "mute_role")
    )
...
class ModCog(Cog, name="Mod Tools"):
    ...
    async def mod_loop(self):
        ...
        for ban in await run_in_thread(db.query, Ban, active=True):
            if ban.days != -1 and datetime.utcnow() >= ban.timestamp + timedelta(
                days=ban.days
            ):
                ...
                await send_to_changelog(
                    guild,
                    translations.f_log_unbanned_expired(
                        f"<@{ban.member}>", ban.member_name
                    ),
                )
        ...
        for mute in await run_in_thread(db.query, Mute, active=True):
            if mute.days != -1 and datetime.utcnow() >= mute.timestamp + timedelta(
                days=mute.days
            ):
                member: Optional[Member] = guild.get_member(mute.member)
                if member is not None:
                ...
                await send_to_changelog(
                    guild,
                    translations.f_log_unmuted_expired(
                        f"<@{mute.member}>", mute.member_name
                    ),
                )
    ...
    async def on_member_join(self, member: Member):
        ...
        if (
            await run_in_thread(db.first, Mute, active=True, member=member.id)
            is not None
        ):
            await member.add_roles(mute_role)
    ...
    async def on_member_update(self, before: Member, after: Member):
        ...
        await run_in_thread(
            UsernameUpdate.create, before.id, str(before), after.nick, True
        )
        return True
    ...
    async def on_user_update(self, before: User, after: User):
        ...
        await run_in_thread(
            UsernameUpdate.create, before.id, str(before), str(after), False
        )
        return True
    ...
    async def report(self, ctx: Context, member: Member, *, reason: str):
        ...
        await run_in_thread(
            Report.create, member.id, str(member), ctx.author.id, reason
        )
        await ctx.send(translations.reported_response)
        await send_to_changelog(
            ctx.guild,
            translations.f_log_reported(
    ...
    async def warn(self, ctx: Context, member: Member, *, reason: str):
        ...
        try:
            await member.send(
                translations.f_warned(ctx.author.mention, ctx.guild.name, reason)
            )
        ...
        await send_to_changelog(
            ctx.guild,
            translations.f_log_warned(
                ctx.author.mention, member.mention, member, reason
            ),
        )

    async def get_user(
        self, guild: Guild, user: Union[Member, User, int]
    ) -> Union[Member, User]:
        if isinstance(user, int):
            if (
                user := guild.get_member(user) or await self.bot.fetch_user(user)
            ) is None:
    ...
    async def mute(
        ...
        *,
        ...
        try:
            if days is not None:
                await user.send(
                    translations.f_muted(
                        ctx.author.mention, ctx.guild.name, days, reason
                    )
                )
            else:
                await user.send(
                    translations.f_muted_inf(ctx.author.mention, ctx.guild.name, reason)
                )
        except (Forbidden, HTTPException):
            await ctx.send(translations.no_dm)
        if days is not None:
            await run_in_thread(
                Mute.create, user.id, str(user), ctx.author.id, days, reason
            )
            await ctx.send(translations.muted_response)
            await send_to_changelog(
                ctx.guild,
                translations.f_log_muted(
                    ctx.author.mention, user.mention, user, days, reason
                ),
            )
        else:
            await run_in_thread(
                Mute.create, user.id, str(user), ctx.author.id, -1, reason
            )
            await ctx.send(translations.muted_response)
            await send_to_changelog(
                ctx.guild,
                translations.f_log_muted_inf(
                    ctx.author.mention, user.mention, user, reason
                ),
            )
    ...
    @guild_only()
    async def unmute(
        self, ctx: Context, user: Union[Member, User, int], *, reason: str
    ):
        """
        unmute a member
        """
        ...
        await send_to_changelog(
            ctx.guild,
            translations.f_log_unmuted(ctx.author.mention, user.mention, user, reason),
        )
    ...
    async def kick(self, ctx: Context, member: Member, *, reason: str):
        ...
        try:
            await member.send(
                translations.f_kicked(ctx.author.mention, ctx.guild.name, reason)
            )
        except (Forbidden, HTTPException):
            await ctx.send(translations.no_dm)
        await member.kick()
    ...
    async def ban(
        ...
        *,
        ...
        if isinstance(user, Member) and (
            user.top_role >= ctx.guild.me.top_role or user.id == ctx.guild.owner_id
        ):
        ...
        try:
            if days is not None:
                await user.send(
                    translations.f_banned(
                        ctx.author.mention, ctx.guild.name, days, reason
                    )
                )
            else:
                await user.send(
                    translations.f_banned_inf(
        ...
        if days is not None:
            await run_in_thread(
                Ban.create, user.id, str(user), ctx.author.id, days, reason
            )
            await ctx.send(translations.banned_response)
            await send_to_changelog(
                ctx.guild,
                translations.f_log_banned(
                    ctx.author.mention, user.mention, user, days, reason
                ),
            )
        else:
            await run_in_thread(
                Ban.create, user.id, str(user), ctx.author.id, -1, reason
            )
            await ctx.send(translations.banned_response)
            await send_to_changelog(
                ctx.guild,
                translations.f_log_banned_inf(
                    ctx.author.mention, user.mention, user, reason
                ),
            )
    ...
    async def unban(self, ctx: Context, user: Union[User, int], *, reason: str):
        ...
        await send_to_changelog(
            ctx.guild,
            translations.f_log_unbanned(ctx.author.mention, user.mention, user, reason),
        )

    async def get_stats_user(
        self, author: User, user: Optional[Union[User, int]]
    ) -> Tuple[Union[User, int], int]:
        if user is None:
            user = author
        ...
        if user_id != author.id and not await check_permissions(
            author, Permission.view_stats
        ):
    ...
    async def stats(self, ctx: Context, user: Optional[Union[User, int]] = None):
        ...
        if (
            ban := await run_in_thread(db.first, Ban, member=user_id, active=True)
        ) is not None:
            if ban.days != -1:
            ...
            mute := await run_in_thread(db.first, Mute, member=user_id, active=True)
        ) is not None:
            if mute.days != -1:
            ...
            status = translations.f_member_since(
    ...
    async def userlogs(self, ctx: Context, user: Optional[Union[User, int]] = None):
        ...
        for username_update in await run_in_thread(
            db.query, UsernameUpdate, member=user_id
        ):
            if not username_update.nick:
                out.append(
                    (
                        username_update.timestamp,
                        translations.f_ulog_username_updated(username_update.new_name),
                    )
                )
                ...
                out.append(
                    (
                        username_update.timestamp,
                        translations.f_ulog_nick_updated(username_update.new_name),
                    )
                )
        for report in await run_in_thread(db.query, Report, member=user_id):
            out.append(
                (
                    report.timestamp,
                    translations.f_ulog_reported(
                        f"<@{report.reporter}>", report.reason
                    ),
                )
            )
        for warn in await run_in_thread(db.query, Warn, member=user_id):
            out.append(
                (
                    warn.timestamp,
                    translations.f_ulog_warned(f"<@{warn.mod}>", warn.reason),
                )
            )
        for mute in await run_in_thread(db.query, Mute, member=user_id):
            if mute.days == -1:
                out.append(
                    (
                        mute.timestamp,
                        translations.f_ulog_muted_inf(f"<@{mute.mod}>", mute.reason),
                    )
                )
            else:
                out.append(
                    (
                        mute.timestamp,
                        translations.f_ulog_muted(
                            f"<@{mute.mod}>", mute.days, mute.reason
                        ),
                    )
                )
            if not mute.active:
                if mute.unmute_mod is None:
                    out.append(
                        (mute.deactivation_timestamp, translations.ulog_unmuted_expired)
                    )
                else:
                    out.append(
                        (
                            mute.deactivation_timestamp,
                            translations.f_ulog_unmuted(
                                f"<@{mute.unmute_mod}>", mute.unmute_reason
                            ),
                        )
                    )
        for kick in await run_in_thread(db.query, Kick, member=user_id):
            out.append(
                (
                    kick.timestamp,
                    translations.f_ulog_kicked(f"<@{kick.mod}>", kick.reason),
                )
            )
        for ban in await run_in_thread(db.query, Ban, member=user_id):
            if ban.days == -1:
                out.append(
                    (
                        ban.timestamp,
                        translations.f_ulog_banned_inf(f"<@{ban.mod}>", ban.reason),
                    )
                )
            else:
                out.append(
                    (
                        ban.timestamp,
                        translations.f_ulog_banned(
                            f"<@{ban.mod}>", ban.days, ban.reason
                        ),
                    )
                )
            if not ban.active:
                if ban.unban_mod is None:
                    out.append(
                        (ban.deactivation_timestamp, translations.ulog_unbanned_expired)
                    )
                else:
                    out.append(
                        (
                            ban.deactivation_timestamp,
                            translations.f_ulog_unbanned(
                                f"<@{ban.unban_mod}>", ban.unban_reason
                            ),
                        )
                    )
        for log in await run_in_thread(
            db.query, InviteLog, applicant=user_id
        ):  # type: InviteLog
            if log.approved:
                out.append(
                    (
