...
async def gather_roles(guild: Guild, channel_id: int) -> List[Role]:
    return [
        ...
        for link in await run_in_thread(
            db.query, RoleVoiceLink, voice_channel=channel_id
        )
...
) -> Tuple[Optional[DynamicVoiceGroup], Optional[DynamicVoiceChannel]]:
    dyn_channel: DynamicVoiceChannel = await run_in_thread(
        db.first, DynamicVoiceChannel, channel_id=channel.id
    )
    if dyn_channel is not None:
        group = await run_in_thread(db.get, DynamicVoiceGroup, dyn_channel.group_id)
    else:
        group = await run_in_thread(db.first, DynamicVoiceGroup, channel_id=channel.id)
...
class VoiceChannelCog(Cog, name="Voice Channels"):
    ...
    async def on_ready(self) -> bool:
        ...
        for link in await run_in_thread(db.query, RoleVoiceLink):
            ...
            if role is not None and voice is not None:
                ...
                if group is not None:
                    for dyn_channel in await run_in_thread(
                        db.query, DynamicVoiceChannel, group_id=group.id
                    ):
                        channel: Optional[VoiceChannel] = guild.get_channel(
                            dyn_channel.channel_id
                        )
        ...
        for group in await run_in_thread(db.query, DynamicVoiceGroup):
            ...
            for dyn_channel in await run_in_thread(
                db.all, DynamicVoiceChannel, group_id=group.id
            ):
                channel: Optional[VoiceChannel] = guild.get_channel(
                    dyn_channel.channel_id
                )
                if channel is not None and not channel.members:
                    ...
                    if (
                        text_channel := self.bot.get_channel(dyn_channel.text_chat_id)
                    ) is not None:
    ...
    async def get_dynamic_voice_channel(
        ...
        DynamicVoiceGroup, DynamicVoiceChannel, VoiceChannel, Optional[TextChannel]
    ]:
        ...
        if dyn_channel is None:
            raise CommandError(translations.not_in_private_voice)
        group: DynamicVoiceGroup = await run_in_thread(
            db.get, DynamicVoiceGroup, dyn_channel.group_id
        )
    ...
    async def member_join(
        ...
        if (
            ...
            and len(channel.category.channels) >= 49
            or len(channel.guild.channels) >= 499
        ):
        ...
        number = (
            len(await run_in_thread(db.all, DynamicVoiceChannel, group_id=group.id)) + 1
        )
        ...
        if (
            team_role := guild.get_role(
                await run_in_thread(Settings.get, int, "team_role")
            )
        ) is not None:
        ...
        text_chat: TextChannel = await category.create_text_channel(
            chan.name, overwrites=overwrites
        )
        ...
        if not group.public:
            await chan.edit(
                overwrites={
                    **overwrites,
                    member: PermissionOverwrite(read_messages=True, connect=True),
                }
            )
        try:
            ...
            await run_in_thread(
                DynamicVoiceChannel.create, chan.id, group.id, text_chat.id, member.id
            )
        await self.update_dynamic_voice_group(group)
        await text_chat.send(translations.f_dyn_voice_created(member.mention))
        if not group.public:
            await text_chat.send(
                translations.f_private_dyn_voice_help(prefix=await get_prefix())
            )
    ...
    async def member_leave(
        ...
        if not group.public and member.id == dyn_channel.owner and len(members) > 0:
            ...
            await run_in_thread(
                DynamicVoiceChannel.change_owner, dyn_channel.channel_id, new_owner.id
            )
            if text_chat is not None:
                await text_chat.send(
    ...
    async def update_dynamic_voice_group(self, group: DynamicVoiceGroup):
        ...
        for dyn_channel in await run_in_thread(
            ...
            channel: Optional[VoiceChannel] = self.bot.get_channel(
                dyn_channel.channel_id
            )
            text_chat: Optional[TextChannel] = self.bot.get_channel(
                dyn_channel.text_chat_id
    ...
    async def list_dyn(self, ctx: Context):
        ...
        for group in await run_in_thread(db.all, DynamicVoiceGroup):
            cnt = len(
                await run_in_thread(db.all, DynamicVoiceChannel, group_id=group.id)
            )
            ...
            out.append(
                f"- [{['private', 'public'][group.public]}] "
                + translations.f_group_list_entry(group.name, cnt)
            )
    ...
    async def add_dyn(
        self, ctx: Context, visibility: str, *, voice_channel: VoiceChannel
    ):
        """
        ...
        if (
            await run_in_thread(db.get, DynamicVoiceChannel, voice_channel.id)
            is not None
        ):
            raise CommandError(translations.dyn_group_already_exists)
        if (
            await run_in_thread(
                db.first, DynamicVoiceGroup, channel_id=voice_channel.id
            )
            is not None
        ):
            raise CommandError(translations.dyn_group_already_exists)

        name: str = re.match(r"^(.*?) ?\d*$", voice_channel.name).group(
            1
        ) or voice_channel.name
    ...
    async def remove_dyn(self, ctx: Context, *, voice_channel: VoiceChannel):
        ...
        await send_to_changelog(
            ctx.guild, translations.f_log_dyn_group_removed(group.name)
        )
    ...
    async def close(self, ctx: Context):
        ...
        (
            ...
            voice_channel,
            text_channel,
        ) = await self.get_dynamic_voice_channel(ctx.author, True)
        await run_in_thread(db.delete, dyn_channel)
    ...
    async def owner(self, ctx: Context, member: Optional[Member]):
        ...
        (
            ...
            voice_channel,
            text_channel,
        ) = await self.get_dynamic_voice_channel(ctx.author, change)

        if not change:
            await ctx.send(
                translations.f_owner_of_private_voice(f"<@{dyn_channel.owner}>")
            )
            return
        ...
        await run_in_thread(
            DynamicVoiceChannel.change_owner, dyn_channel.channel_id, member.id
        )
        if text_channel is not None:
            await text_channel.send(
    ...
    async def create_link(self, ctx: Context, channel: VoiceChannel, *, role: Role):
        ...
        if (
            await run_in_thread(
                db.first, RoleVoiceLink, role=role.id, voice_channel=channel.id
            )
            is not None
        ):
        ...
        if role > ctx.me.top_role:
            raise CommandError(
                translations.f_link_not_created_too_high(role, ctx.me.top_role)
            )
        if role.managed:
        ...
        group: Optional[DynamicVoiceGroup] = await run_in_thread(
            db.first, DynamicVoiceGroup, channel_id=channel.id
        )
        if group is not None:
            for dyn_channel in await run_in_thread(
                db.query, DynamicVoiceChannel, group_id=group.id
            ):
                dchannel: Optional[VoiceChannel] = self.bot.get_channel(
                    dyn_channel.channel_id
                )
    ...
    async def remove_link(self, ctx: Context, channel: VoiceChannel, *, role: Role):
        ...
        await send_to_changelog(
            ctx.guild, translations.f_log_link_deleted(channel, role)
        )
