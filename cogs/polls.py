...
default_emojis = [emoji_map[f"regional_indicator_{x}"] for x in "abcdefghijklmnopqrstuvwyz"]
...
class PollsCog(Cog, name="Polls"):
    ...
    async def yesno(self, ctx: Context, message: Optional[Message] = None):
        ...
        if message.channel.permissions_for(ctx.author).add_reactions:
            await message.add_reaction(emoji_map["thumbsup"])
            await message.add_reaction(emoji_map["thumbsdown"])
...
class PollOption:
    def __init__(self, ctx: Context, line: str, number: int):
        ...
        elif (unicode_emoji := emoji_candidate) in emoji_map.values():
